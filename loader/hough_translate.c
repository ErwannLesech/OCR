...
struct Line make_line(double theta, double rho, int w, int h, int val){
    ...
    if (theta < M_PI * 0.25 || theta > M_PI * 0.75) { 
      ...
      x1=(int) ((((rho - houghHeight) - ((y1 - centerY) * tsin)) / tcos) + centerX);
      x2=(int) ((((rho - houghHeight) - ((y2 - centerY) * tsin)) / tcos) + centerX);
struct Line line = { .x_start = x1, .y_start = y1, .x_end = x2, .y_end = y2, .val = val};
    return line;
    }
    else {
      int x1=0, y1=0;
      ...
      y1=(int) ((((rho - houghHeight) - ((x1 - centerX) * tcos)) / tsin) + centerY); 
      y2=(int) ((((rho - houghHeight) - ((x2 - centerX) * tcos)) / tsin) + centerY); 
      struct Line line = { .x_start = x1, .y_start = y1, .x_end = x2, .y_end = y2, .val = val};
    return line;
}
}
...
struct Accum_in_mat** Matrix(int h, int w){
    struct Accum_in_mat **t = (struct Accum_in_mat **)malloc(h * sizeof(struct Accum_in_mat *));
...
struct Lines Hough(SDL_Surface *sdl_surface){
      ...
      const double diagonal = (int) (sqrt(2) * fmax(sdl_surface->h, sdl_surface->w)) / 2; 
    ...
    int num_points = 0;
        ...
        for (int x = 0; x < sdl_surface->w; x++)
        {
            int i = 0;
           Uint8 r,g,b;
           SDL_GetRGB(pixels[y*(sdl_surface->w) + x], sdl_surface->format, &r, &g,&b);
            if (r ==255)
            {
                //printf("+1\n");
                for (int theta = 0; theta < 180; theta++)
                {
                    double rho = (x-centerX)*saveCos[theta]+ (y-centerY)*saveSin[theta];
    ...
    for (size_t t = 0; t < 180; t++)
    {
        ...
        //struct Line ligne = make_line(theta, r, sdl_surface->w, sdl_surface->h, val);
             lines[e] = make_line(theta, r, sdl_surface->w, sdl_surface->h, val);
            e++;
            }
            }
        }
...
sort_Lines(lines, e);
    for (size_t j = 0; j < e; j++)
    {
      ...
      printf("x:%i,%i and y:%i,%i, val = %i \n", line.x_start, line.x_end, line.y_start, line.y_end,line.val);
    }
